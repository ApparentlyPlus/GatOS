def extract_page_table_indices(virtual_address):
    ...
    print(f"  PML4 Index : 0x{pml4_index:04X} ({pml4_index})")
    print(f"  PDPT Index : 0x{pdpt_index:04X} ({pdpt_index})")
    print(f"  PD   Index : 0x{pd_index:04X} ({pd_index})")
    print(f"  PT   Index : 0x{pt_index:04X} ({pt_index})")
