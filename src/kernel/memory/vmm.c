...
vmm_status_t vmm_alloc(vmm_t* vmm_pub, size_t length, size_t flags, void* arg, void** out_addr) {
    ...
    *out_addr = NULL;
    ...
    size_t virt_align = (!(flags & (VM_FLAG_FOREIGN_PHYS | VM_FLAG_LAZY)) && length >= PAGE_2MB)
                        ? PAGE_2MB : PAGE_SIZE;
    ...
    obj->phys_base   = (flags & VM_FLAG_FOREIGN_PHYS) ? VMM_PHYS_NONE : phys_base;
    obj->phys_length = (flags & VM_FLAG_FOREIGN_PHYS) ? 0 : length;
    bool is_user_vmm = !vmm->is_kernel;
    uint64_t pt_flags = vmm_convert_vm_flags(flags, vmm->is_kernel);
    bool allow_huge = !(flags & (VM_FLAG_FOREIGN_PHYS | VM_FLAG_LAZY));
    bool any_huge = false;
    ...
    while (offset < length) {
        ...
        if (ms != VMM_OK) {
            ...
            if (!(flags & VM_FLAG_FOREIGN_PHYS)) pmm_free(phys_base, length);
            vma_remove(vmm, obj);
            vmm_free_vm_object(obj);
            spinlock_release(&vmm->lock, lock_flags);
            return ms;
...
vmm_status_t vmm_free(vmm_t* vmm_pub, void* addr) {
    ...
    bool has_pmm_backing = !(cur->public.flags & VM_FLAG_FOREIGN_PHYS);
...
void vmm_destroy(vmm_t* vmm_pub) {
    ...
    while (n) {
        ...
        if (!(cur->public.flags & VM_FLAG_FOREIGN_PHYS)) {
            if (cur->pg_size > PAGE_SIZE) {
...
vmm_status_t vmm_resize(vmm_t* vmm_pub, void* addr, size_t new_length) {
    ...
    if ((cur->public.flags & VM_FLAG_FOREIGN_PHYS) || cur->pg_size > PAGE_SIZE) {
