...
void lapic_init(void) {
    ...
    if (lapic_base == 0) {
        void* virt_addr = NULL;
        if (vmm_alloc(NULL, PAGE_SIZE, VM_FLAG_WRITE | VM_FLAG_FOREIGN | VM_FLAG_DEVICE, (void*)phys_base, &virt_addr) != VMM_OK)
...
void ioapic_init(void) {
    ...
    void* virt_addr = NULL;
    if (vmm_alloc(NULL, PAGE_SIZE, VM_FLAG_WRITE | VM_FLAG_FOREIGN | VM_FLAG_DEVICE, (void*)phys, &virt_addr) != VMM_OK)
